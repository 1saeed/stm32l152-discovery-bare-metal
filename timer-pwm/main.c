 ...
 *
 * Purpose:   
 *  to generate 60Hz PWM output on PC6 (TIM3_CH1)
 *  and then capture this signal on PA15 (TIM2_CH1) on rising edge
 *  add watch a variable to monitor the frequency
 *
 *
 * Configuration:
 * pwm mode:
...
#include <stdio.h>
	 ...
	 { 
		  TIM2->CNT = 0;																				// reset counter
			while(!((TIM2->SR & 0x00000002) == 0x00000002)){}			// wait until an edge has been detected on PA15 (TIM2_CH1) which matches the selected polarity
					period = TIM2->CCR1;											  			// CCR1 is the counter value transferred by the last input capture 1 event (IC1).
 ...
 { 
	 ...
	 TIM3->PSC      = 10 - 1;					  // divided by 10 --> 1.6MHz @16MHz
	 TIM3->ARR      = 26667 - 1;				// ~60 HZ PWM frequency
	 
	 TIM3->CNT			= 0;								// reset timer counter
	 
	 TIM3->CCMR1    = 0x0060;						// PWM mode 1 in upcounting, channel 1 is active as long as TIMx_CNT<TIMx_CCR1 else inactive
	 TIM3->CCER		 |= 0x00000001;				// PWM enable ch1
	 
	 TIM3->CCR1     = 8889 - 1;					// set match mode for ch1 for 30% duty cycle
 ...
 void tim2_input_capture_init(void)
 {
	 ...
	 TIM2->PSC     = 16 - 1;						// divided by 16 --> 1MHz @16MHz
